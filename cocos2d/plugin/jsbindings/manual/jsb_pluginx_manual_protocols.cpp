...
bool js_pluginx_protocols_PluginParam_constructor( JSContext *cx, uint32_t argc, jsval *vp ) {
    ...
    do {
        if ( argc == 2 ) {
            ...
            assert( typeClass );
            obj = JS_NewObject( cx, typeClass->jsclass, JS::RootedObject( cx, typeClass->proto ),
                                JS::RootedObject( cx, typeClass->parentProto ) );
            js_proxy_t *proxy;
...
bool js_pluginx_PluginProtocol_callFuncWithParam( JSContext *cx, uint32_t argc, jsval *vp ) {
    ...
    JSB_PRECONDITION2( cobj, cx, false,
                       "js_pluginx_protocols_PluginProtocol_callFuncWithParam : Invalid Native Object" );
