...
// JSFunctionWrapper
    JSFunctionWrapper::JSFunctionWrapper( JSContext *cx, JSObject *jsthis, jsval fval )
            : _cx( cx ), _jsthis( jsthis ), _fval( fval ) {
    ...
    bool JSFunctionWrapper::invoke( unsigned int argc, jsval *argv, JS::MutableHandleValue rval ) {
        ...
        return JS_CallFunctionValue( this->_cx, JS::RootedObject( _cx, this->_jsthis ),
                                     JS::RootedValue( _cx, this->_fval ),
    ...
    bool jsval_to_long_long( JSContext *cx, JS::HandleValue vp, long long *r ) {
        ...
        JSB_PRECONDITION2( JS_GetTypedArrayByteLength( tmp_arg ) == sizeof( long long ), cx, false,
...
// From native type to jsval
    ...
    jsval TProductInfo_to_jsval( JSContext *cx, TProductInfo &ret ) {
        ...
        for ( TProductInfo::iterator it = ret.begin(); it != ret.end(); ++it ) {
            JS_DefineProperty( cx, tmp, it->first.c_str(), JS::RootedValue( cx, std_string_to_jsval( cx, it->second ) ),
    ...
    jsval LogEventParamMap_to_jsval( JSContext *cx, LogEventParamMap *&ret ) {// TODO:
        return JSVAL_NULL;
    }
