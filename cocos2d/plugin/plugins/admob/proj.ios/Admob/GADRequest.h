...
- (void)
registerAdNetworkExtras: ( id < GADAdNetworkExtras > )
extras;

/// Get the network extras defined for an ad network.
- (id <GADAdNetworkExtras>)
adNetworkExtrasFor: ( Class < GADAdNetworkExtras > )
aClass;

/// Unsets the extras for an ad network. |clazz| is the class which represents that network's extras
/// type.
- (void)
removeAdNetworkExtrasFor: ( Class < GADAdNetworkExtras > )
aClass;
...
NSDate *birthday;
- (void)
setBirthdayWithMonth: ( NSInteger )
m day:
(NSInteger)
d year:
(NSInteger)
...
- (void)
setLocationWithLatitude: ( CGFloat )
latitude
        longitude:
(CGFloat)
longitude
...
/// @"94041 US".
- (void)
setLocationWithDescription: ( NSString * )
locationDescription;
...
NSDictionary *additionalParameters __attribute__((
...
@
property(nonatomic, assign, getter = isTesting
)
BOOL testing
