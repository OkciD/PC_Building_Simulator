...
inline void
resolveSingleConstraintRowGeneric( PfxSolverBody &body1, PfxSolverBody &body2, const btSolverConstraint &c ) {

    btScalar deltaImpulse = c.m_rhs - btScalar( c.m_appliedImpulse ) * c.m_cfm;
    const btScalar deltaVel1Dotn = c.m_contactNormal1.dot( getBtVector3( body1.mDeltaLinearVelocity ) ) +
                                   c.m_relpos1CrossNormal.dot( getBtVector3( body1.mDeltaAngularVelocity ) );
    const btScalar deltaVel2Dotn = c.m_contactNormal2.dot( getBtVector3( body2.mDeltaLinearVelocity ) ) +
                                   c.m_relpos2CrossNormal.dot( getBtVector3( body2.mDeltaAngularVelocity ) );
//	const btScalar delta_rel_vel	=	deltaVel1Dotn-deltaVel2Dotn;
    ...
    if ( body1.mMassInv ) {
        ...
        body1.mDeltaLinearVelocity += vmVector3( linearComponent.getX() * deltaImpulse,
                                                 linearComponent.getY() * deltaImpulse,
                                                 linearComponent.getZ() * deltaImpulse );
    ...
    if ( body2.mMassInv ) {
        ...
        body2.mDeltaLinearVelocity += vmVector3( linearComponent.getX() * deltaImpulse,
                                                 linearComponent.getY() * deltaImpulse,
                                                 linearComponent.getZ() * deltaImpulse );
        btVector3 tmp = c.m_angularComponentB *
                        ( ( btVector3( deltaImpulse, deltaImpulse, deltaImpulse ) ) );//*m_angularFactor);
...
void pfxSolveLinearConstraintRow( btConstraintRow &constraint,
    ...
    constraint.m_accumImpulse = btClamped( oldImpulse + deltaImpulse, constraint.m_lowerLimit,
...
void btSolveContactConstraint(
    ...
    pfxSolveLinearConstraintRow( constraintResponse,
                                 solverBodyA.mDeltaLinearVelocity, solverBodyA.mDeltaAngularVelocity,
                                 solverBodyA.mMassInv, solverBodyA.mInertiaInv, rA,
                                 solverBodyB.mDeltaLinearVelocity, solverBodyB.mDeltaAngularVelocity,
                                 solverBodyB.mMassInv, solverBodyB.mInertiaInv, rB );
                        ...
                        for ( int j = 0; j < contact.getNumContacts(); j++ ) {
                            ...
                            if ( k == 0 ) {
                                ...
                                for ( int k = 0; k < 3; k++ ) {
                                    ...
                                    solverBodyA.mDeltaAngularVelocity +=
                                            deltaImpulse * solverBodyA.mInertiaInv * cross( rA, normal );
                                    solverBodyB.mDeltaLinearVelocity -= deltaImpulse * solverBodyB.mMassInv * normal;
                                    solverBodyB.mDeltaAngularVelocity -=
                                            deltaImpulse * solverBodyB.mInertiaInv * cross( rB, normal );
                                }
                            } else {
...
//	constraintFriction2.m_accumImpulse = 0.f;
    ...
    // Contact Constraint
    {
        ...
        constraintResponse.m_rhs -=
                ( separateBias * btMin( 0.0f, penetrationDepth + PFX_CONTACT_SLOP ) ) / timeStep; // position error
...
void CustomSetupContactConstraintsTask(
    ...
    for ( uint32_t i = 0; i < numContactPairs; i++ ) {
        ...
        if ( !pfxGetActive( pair ) || pfxGetNumConstraints( pair ) == 0 ||
             ( ( pfxGetMotionMaskA( pair ) & PFX_MOTION_MASK_STATIC ) &&
               ( pfxGetMotionMaskB( pair ) & PFX_MOTION_MASK_STATIC ) ) ) {
            continue;
        }
...
void CustomSplitConstraints(
    ...
    // �ڕW�Ƃ��镪����
    uint32_t targetCount = btMax( uint32_t( PFX_MIN_SOLVER_PAIRS ),
                                  btMin( numPairs / ( numTasks * 2 ), uint32_t( PFX_MAX_SOLVER_PAIRS ) ) );
        ...
        for ( batchId = 0; i < numPairs && totalCount < numPairs && batchId < maxBatches; batchId++ ) {
            ...
            for ( ; i < numPairs && pairCount < targetCount; i++ ) {
                ...
                if ( !pfxGetActive( pairs[i] ) || pfxGetNumConstraints( pairs[i] ) == 0 ||
                     ( ( pfxGetMotionMaskA( pairs[i] ) & PFX_MOTION_MASK_STATIC ) &&
                       ( pfxGetMotionMaskB( pairs[i] ) & PFX_MOTION_MASK_STATIC ) ) ) {
                    if ( startIndexCheck )
                        startIndex++;
...
//	config.taskManager->setTaskEntry(PFX_SOLVER_ENTRY);
...

    {
        ...
        PfxParallelBatch *cbatches = ( PfxParallelBatch * ) pool.allocate(
                sizeof( PfxParallelBatch ) * ( PFX_MAX_SOLVER_PHASES * PFX_MAX_SOLVER_BATCHES ), 128 );
        PfxParallelGroup *jgroup = ( PfxParallelGroup * ) pool.allocate( sizeof( PfxParallelGroup ) );
        PfxParallelBatch *jbatches = ( PfxParallelBatch * ) pool.allocate(
                sizeof( PfxParallelBatch ) * ( PFX_MAX_SOLVER_PHASES * PFX_MAX_SOLVER_BATCHES ), 128 );

        uint32_t tmpBytes = poolBytes - 2 * ( sizeof( PfxParallelGroup ) + sizeof( PfxParallelBatch ) *
                                                                           ( PFX_MAX_SOLVER_PHASES *
                                                                             PFX_MAX_SOLVER_BATCHES ) + 128 );
        void *tmpBuff = pool.allocate( tmpBytes );

        {
            BT_PROFILE( "CustomSplitConstraints" );
            CustomSplitConstraints( contactPairs, numContactPairs, *cgroup, cbatches, maxTasks, numRigidBodies, tmpBuff,
                                    tmpBytes );
            CustomSplitConstraints( jointPairs, numJointPairs, *jgroup, jbatches, maxTasks, numRigidBodies, tmpBuff,
                                    tmpBytes );
        }
...
btScalar btParallelConstraintSolver::solveGroup( btCollisionObject **bodies1, int numRigidBodies,
...

    {
        ...
        for ( int i = 0; i < numRigidBodies; i++ ) {
            ...
            if ( rb && ( rb->getInvMass() > 0.f ) ) {
                btVector3 angVelPlusForces = rb->getAngularVelocity() +
                                             rb->getTotalTorque() * rb->getInvInertiaTensorWorld() *
                                             infoGlobal.m_timeStep;
                btVector3 linVelPlusForces =
                        rb->getLinearVelocity() + rb->getTotalForce() * rb->getInvMass() * infoGlobal.m_timeStep;
            ...
            for ( i = 0; i < numConstraints; i++ ) {
                ...
                if ( info1.m_numConstraintRows ) {
                    ...
                    for ( j = 0; j < info1.m_numConstraintRows; j++ ) {
                        ...
                        {
                            btScalar rel_vel;
                            btScalar vel1Dotn = solverConstraint.m_contactNormal1.dot( rbA.getLinearVelocity() ) +
                                                solverConstraint.m_relpos1CrossNormal.dot( rbA.getAngularVelocity() );
                            btScalar vel2Dotn = solverConstraint.m_contactNormal2.dot( rbB.getLinearVelocity() ) +
...
//			PFX_PRINTF("num points PFX = %d\n",total);
    ...
    //copy results back to bodies
    {
        ...
        for ( int i = 0; i < numRigidBodies; i++ ) {
            ...
            TrbState &state = m_memoryCache->m_mystates[i];
            if ( rb && ( rb->getInvMass() > 0.f ) ) {
                rb->setLinearVelocity( btVector3( state.getLinearVelocity().getX(), state.getLinearVelocity().getY(),
                                                  state.getLinearVelocity().getZ() ) );
                rb->setAngularVelocity( btVector3( state.getAngularVelocity().getX(), state.getAngularVelocity().getY(),
                                                   state.getAngularVelocity().getZ() ) );
            }
        }
    }
