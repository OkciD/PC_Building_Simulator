...
BT_GPU___device__ uint bt3DGrid_calcGridHash( int3 gridPos ) {
    ...
    return BT_GPU___mul24( BT_GPU___mul24( gridPos.z, BT_GPU_params.m_gridSizeY ), BT_GPU_params.m_gridSizeX ) +
           BT_GPU___mul24( gridPos.y, BT_GPU_params.m_gridSizeX ) + gridPos.x;
...
BT_GPU___device__ uint
cudaTestAABBOverlap( bt3DGrid3F1U min0, bt3DGrid3F1U max0, bt3DGrid3F1U min1, bt3DGrid3F1U max1 ) {
    ...
    for ( uint index2 = bucketStart; index2 < bucketEnd; index2++ ) {
        ...
        if ( unsorted_indx2 < unsorted_indx ) // check not colliding with self
        {
            ...
            if ( cudaTestAABBOverlap( min0, max0, min1, max1 ) ) {
                ...
                if ( k == curr ) {
                    if ( curr >= curr_max ) { // not a good solution, but let's avoid crash
                        break;
                    }
...
BT_GPU___global__ void findOverlappingPairsD( bt3DGrid3F1U *pAABB, uint2 *pHash, uint *pCellStart,
    ...
    for ( int z = -1; z <= 1; z++ ) {
        for ( int y = -1; y <= 1; y++ ) {
            for ( int x = -1; x <= 1; x++ ) {
                findPairsInCell( gridPos + BT_GPU_make_int3( x, y, z ), index, pHash, pCellStart, pAABB, pPairBuff,
                                 pPairBuffStartCurr, numBodies );
            }
        }
    }
...
void BT_GPU_PREF( findCellStart( unsigned
                          int *hash,
...
void BT_GPU_PREF( findOverlappingPairs( bt3DGrid3F1U * pAABB, unsigned
                          int *pHash,
    ...
    BT_GPU_EXECKERNEL( numBlocks, numThreads, findOverlappingPairsD,
                       ( pAABB, ( uint2 * ) pHash, ( uint * ) pCellStart, ( uint * ) pPairBuff, ( uint2 * ) pPairBuffStartCurr, numBodies ) );
...
void BT_GPU_PREF( findPairsLarge( bt3DGrid3F1U * pAABB, unsigned
                          int *pHash,
                          unsigned int *pCellStart,
                          unsigned int *pPairBuff,
                          unsigned int *pPairBuffStartCurr,
    ...
    int numThreads, numBlocks;
    BT_GPU_PREF( computeGridSize )( numBodies, 64, numBlocks, numThreads );
    BT_GPU_EXECKERNEL( numBlocks, numThreads, findPairsLargeD,
                       ( pAABB, ( uint2 * ) pHash, ( uint * ) pCellStart, ( uint * ) pPairBuff, ( uint2 * ) pPairBuffStartCurr, numBodies, numLarge ) );
...
void BT_GPU_PREF( computePairCacheChanges( unsigned
                          int *pPairBuff,
                          unsigned int *pPairBuffStartCurr,
                          unsigned int *pPairScan, bt3DGrid3F1U
                          *pAABB, unsigned
                          int numBodies) ) {
    int numThreads, numBlocks;
    BT_GPU_PREF( computeGridSize )( numBodies, 256, numBlocks, numThreads );
    BT_GPU_EXECKERNEL( numBlocks, numThreads, computePairCacheChangesD,
                       ( ( uint * ) pPairBuff, ( uint2 * ) pPairBuffStartCurr, ( uint * ) pPairScan, pAABB, numBodies ) );
    BT_GPU_CHECK_ERROR( "Kernel execution failed: btCudaComputePairCacheChangesD" );
} // computePairCacheChanges()

//----------------------------------------------------------------------------------------

void BT_GPU_PREF( squeezeOverlappingPairBuff( unsigned
                          int *pPairBuff,
                          unsigned int *pPairBuffStartCurr,
                          unsigned int *pPairScan,
                          unsigned int *pPairOut, bt3DGrid3F1U
                          *pAABB, unsigned
                          int numBodies) ) {
    int numThreads, numBlocks;
    BT_GPU_PREF( computeGridSize )( numBodies, 256, numBlocks, numThreads );
    BT_GPU_EXECKERNEL( numBlocks, numThreads, squeezeOverlappingPairBuffD,
                       ( ( uint * ) pPairBuff, ( uint2 * ) pPairBuffStartCurr, ( uint * ) pPairScan, ( uint * ) pPairOut, pAABB, numBodies ) );
