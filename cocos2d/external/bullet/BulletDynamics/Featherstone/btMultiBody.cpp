...
namespace {
    void
    SpatialTransform( const btMatrix3x3 &rotation_matrix,  // rotates vectors in 'from' frame to vectors in 'to' frame
...
void btMultiBody::stepVelocities( btScalar dt,
    ...
    btVector3 *vel_top_angular = v_ptr;
    v_ptr += num_links + 1;
    btVector3 *vel_bottom_linear = v_ptr;
    v_ptr += num_links + 1;

    // zhat_i^A
    btVector3 *zero_acc_top_angular = v_ptr;
    v_ptr += num_links + 1;
    btVector3 *zero_acc_bottom_linear = v_ptr;
    v_ptr += num_links + 1;

    // chat_i  (note NOT defined for the base)
    btVector3 *coriolis_top_angular = v_ptr;
    v_ptr += num_links;
    btVector3 *coriolis_bottom_linear = v_ptr;
    v_ptr += num_links;
    ...
    btVector3 *accel_top = v_ptr;
    v_ptr += num_links + 1;
    btVector3 *accel_bottom = v_ptr;
    v_ptr += num_links + 1;
    ...
    if ( fixed_base ) {
        ...
        zero_acc_top_angular[0] = -( rot_from_parent[0] * ( base_force
                                                            - base_mass * ( DAMPING_K1_LINEAR +
                                                                            DAMPING_K2_LINEAR * base_vel.norm() ) *
                                                              base_vel ) );
        ...
        zero_acc_bottom_linear[0] += base_inertia * vel_top_angular[0] *
                                     ( DAMPING_K1_ANGULAR + DAMPING_K2_ANGULAR * vel_top_angular[0].norm() );
    ...
    for ( int i = 0; i < num_links; ++i ) {
        ...
        coriolis_bottom_linear[i] =
                vel_top_angular[i + 1].cross( vel_top_angular[i + 1].cross( links[i].cached_r_vector ) )
        ...
        if ( links[i].is_revolute ) {
            ...
            coriolis_bottom_linear[i] +=
                    ( getJointVel( i ) * getJointVel( i ) ) * links[i].axis_top.cross( links[i].axis_bottom );
        } else {
        ...
        zero_acc_top_angular[i + 1] +=
                links[i].mass * ( DAMPING_K1_LINEAR + DAMPING_K2_LINEAR * vel_bottom_linear[i + 1].norm() ) *
                vel_bottom_linear[i + 1];
        ...
        zero_acc_bottom_linear[i + 1] += links[i].inertia * vel_top_angular[i + 1] *
                                         ( DAMPING_K1_ANGULAR + DAMPING_K2_ANGULAR * vel_top_angular[i + 1].norm() );
    ...
    for ( int i = num_links - 1; i >= 0; --i ) {
        ...
        h_bottom[i] = inertia_bottom_left[i + 1] * links[i].axis_top +
                      inertia_top_left[i + 1].transpose() * links[i].axis_bottom;
        btScalar val = SpatialDotProduct( links[i].axis_top, links[i].axis_bottom, h_top[i], h_bottom[i] );
        D[i] = val;
        Y[i] = links[i].joint_torque
               - SpatialDotProduct( links[i].axis_top, links[i].axis_bottom, zero_acc_top_angular[i + 1],
                                    zero_acc_bottom_linear[i + 1] )
        ...
        const btMatrix3x3 BL =
                inertia_bottom_left[i + 1] - vecMulVecTranspose( one_over_di * h_bottom[i], h_bottom[i] );
        ...
        inertia_top_left[parent + 1] +=
                rot_from_parent[i + 1].transpose() * ( TL - TR * r_cross ) * rot_from_parent[i + 1];
        ...
        inertia_bottom_left[parent + 1] += rot_from_parent[i + 1].transpose() *
                                           ( r_cross * ( TL - TR * r_cross ) + BL - TL.transpose() * r_cross ) *
                                           rot_from_parent[i + 1];
...
//		printf("result=%f,%f,%f,%f,%f,%f\n",result[0],result[0],result[0],result[0],result[0],result[0]);
    ...
    for ( int i = 0; i < num_links; ++i ) {
        ...
        joint_accel[i] =
                ( Y[i] - SpatialDotProduct( h_top[i], h_bottom[i], accel_top[i + 1], accel_bottom[i + 1] ) ) / D[i];
...
void btMultiBody::calcAccelerationDeltas( const btScalar *force, btScalar *output,
    ...
    for ( int i = num_links - 1; i >= 0; --i ) {
        ...
        Y[i] = -SpatialDotProduct( links[i].axis_top, links[i].axis_bottom, zero_acc_top_angular[i + 1],
                                   zero_acc_bottom_linear[i + 1] );
...
void btMultiBody::stepPositions( btScalar dt ) {
    ...
    if ( fabs( omega_times_dt ) < SMALL_ROTATION_ANGLE ) {
        ...
        base_quat = base_quat *
                    btQuaternion( sin_term * base_omega[0], sin_term * base_omega[1], sin_term * base_omega[2],
                                  cos_term );
    } else {
...
void btMultiBody::fillContactJacobian( int link,
    ...
    btVector3 *v_ptr = &scratch_v[0];
    btVector3 *p_minus_com = v_ptr;
    v_ptr += num_links + 1;
    btVector3 *n_local = v_ptr;
    v_ptr += num_links + 1;
    btAssert( v_ptr - &scratch_v[0] == scratch_v.size() );
