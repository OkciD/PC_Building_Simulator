...
void pvmp3_st_intensity_ver2( int32 xr[SUBBANDS_NUMBER * FILTERBANK_BANDS],
    ...
    k[0] = is_pos_pow_eitgh_root_of_2[( ( is_pos + 1 ) & ( 3 + ( m << 2 ) ) ) << ( 1 - m )]
            >> ( ( is_pos + 1 ) >> ( 2 + m ) );
    ...
    if ( i_stereo ) {
        ...
        if ( gr_info_l->window_switching_flag && ( gr_info_l->block_type == 2 ) ) {
            if ( gr_info_l->mixed_block_flag ) {
                ...
                if ( sb < 36 )    /*  was (sb <= 36)  */
                {
                    ...
                    for ( sfb = sfbTemp; sfb < 6; sfb++ ) {
                        ...
                        sfbNo = mp3_sfBandIndex[sfreq].l[sfb + 1] -
                                mp3_sfBandIndex[sfreq].l[sfb]; /* No of lines to process */
                    ...
                    for ( j = 0; j < 3; j++ ) {
                        ...
                        scalefac_IIP_buffer[36 + j] = scalefac_IIP_buffer[33 +
                                                                          j];  /* legal/illegal in sfb 12 same as in sfb 11 */

                        for ( sfb = 3; sfb < 13; sfb++ ) {
                            sfbNo = mp3_sfBandIndex[sfreq].s[sfb + 1] -
                                    mp3_sfBandIndex[sfreq].s[sfb]; /* No of lines to process */
            ...
            for ( sfb = sfbTemp; sfb < 22; sfb++ ) {
                sfbNo = mp3_sfBandIndex[sfreq].l[sfb + 1] -
                        mp3_sfBandIndex[sfreq].l[sfb]; /* number of lines to process */
