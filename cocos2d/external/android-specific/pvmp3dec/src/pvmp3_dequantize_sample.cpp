...
int32 power_1_third( int32 xx ) {
    ...
    if ( xx <= 512 ) {
        ...
        if ( xx >> 15 ) {
            ...
            xx >>= m;
            xx = ( power_one_third[xx] ) +
                 ( ( ( power_one_third[xx + 1] - power_one_third[xx] ) >> m ) * ( x & ( ( 1 << m ) - 1 ) ) );
        ...
        for ( ss = 0; ss < used_freq_lines; ss++ ) {
            if ( ss == next_cb_boundary ) {
                ...
                if ( gr_info->mixed_block_flag ) {
                    ...
                    if ( ss < 2 * FILTERBANK_BANDS ) {   /*  1st 2 subbands of switched blocks */
                        global_gain = ( gr_info->global_gain );
            ...
            if ( ( gr_info->mixed_block_flag == 0 ) ||
                 ( gr_info->mixed_block_flag && ( ss >= 2 * FILTERBANK_BANDS ) ) ) {
                ...
                if ( global_gain <= 0 ) {
                    ...
                    if ( global_gain < 32 ) {
                        ...
                        pv_memset( &is[mp3_sfBandIndex[sfreq].l[cb]],
                                   0,
                                   ( mp3_sfBandIndex[sfreq].l[cb + 1] - mp3_sfBandIndex[sfreq].l[cb] ) *
                                   sizeof( *is ) );
                    }
                } else {
                    for ( ss = mp3_sfBandIndex[sfreq].l[cb]; ss < mp3_sfBandIndex[sfreq].l[cb + 1]; ss += 2 ) {
