...
bool rcBuildHeightfieldLayers( rcContext *ctx, rcCompactHeightfield &chf,
    ...
    rcScopedDelete<unsigned char> srcReg = ( unsigned char * ) rcAlloc( sizeof( unsigned char ) * chf.spanCount,
                                                                        RC_ALLOC_TEMP );
    if ( !srcReg ) {
    ...
    rcScopedDelete<rcLayerSweepSpan> sweeps = ( rcLayerSweepSpan * ) rcAlloc( sizeof( rcLayerSweepSpan ) * nsweeps,
                                                                              RC_ALLOC_TEMP );
    if ( !sweeps ) {
                    ...
                    for ( int dir = 0; dir < 4; ++dir ) {
                        if ( rcGetCon( s, dir ) != RC_NOT_CONNECTED ) {
                            ...
                            if ( chf.areas[ai] != RC_NULL_AREA && lid != alid ) {
                                ...
                                if ( as.y > hmin )
                                    layer->heights[idx] = rcMax( layer->heights[idx],
