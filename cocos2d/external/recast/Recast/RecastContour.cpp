...
bool rcBuildContours( rcContext *ctx, rcCompactHeightfield &chf,
    ...
    rcScopedDelete<unsigned char> flags = ( unsigned char * ) rcAlloc( sizeof( unsigned char ) * chf.spanCount,
                                                                       RC_ALLOC_TEMP );
    if ( !flags ) {
            ...
            for ( int i = ( int ) c.index, ni = ( int ) ( c.index + c.count ); i < ni; ++i ) {
                ...
                if ( simplified.size() / 4 >= 3 ) {
                    if ( cset.nconts >= maxContours ) {
                        ...
                        maxContours *= 2;
                        rcContour *newConts = ( rcContour * ) rcAlloc( sizeof( rcContour ) * maxContours,
                                                                       RC_ALLOC_PERM );
    ...
    if ( cset.nconts > 0 ) {
        ...
        if ( nholes > 0 ) {
            ...
            rcScopedDelete<rcContourHole> holes = ( rcContourHole * ) rcAlloc( sizeof( rcContourHole ) * cset.nconts,
                                                                               RC_ALLOC_TEMP );
            if ( !holes ) {
