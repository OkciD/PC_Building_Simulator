...
+(id)
createMenuItem: (
const player::PlayerMenuItemMac *) macMenuItem
{
if (macMenuItem->
...
-(id)
initWithMenuItem: (
const player::PlayerMenuItemMac *) menuItem
{
NSString *title =
[
...
PlayerMenuItemMac::PlayerMenuItemMac()
        : _parent( nullptr ), _menuItem( nullptr ), _menu( nullptr ) {
}
...
PlayerMenuServiceMac::PlayerMenuServiceMac() {
    ...
    NSApplication * thisApp = [NSApplication
    sharedApplication];
    _root._menu = [thisApp
    mainMenu];
}
...
PlayerMenuItem *PlayerMenuServiceMac::addItem( const std::string &menuId, const std::string &title,
    ...
    if ( !parent->_menu ) {
        NSMenu * nsmenu = [[NSMenu
        alloc] initWithTitle:[parent->_menuItem
        title]];
        [parent->_parent->_menu
        setSubmenu:
        nsmenu
        forItem:
        parent->_menuItem];
    ...
    int childSize = ( int ) [parent->_menu
    itemArray].count;
...
bool PlayerMenuServiceMac::removeItemInternal( const std::string &menuId, bool isUpdateChildrenOrder ) {
    ...
    if ( item->_children.size() == 0 ) {
        ...
        if ( !removed ) {
            CCLOG( "MenuServiceWin::removeItem() - remove menu item (%s) failed, not found command id from parent->children",
                   item->_menuId.c_str() );
        }
