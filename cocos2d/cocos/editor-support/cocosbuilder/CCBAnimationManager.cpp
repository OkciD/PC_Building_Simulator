...
// Implementation of CCBAinmationManager
    ...
    CCBAnimationManager::CCBAnimationManager()
            : _jsControlled( false ), _owner( nullptr ), _autoPlaySequenceId( 0 ), _rootNode( nullptr ),
              _rootContainerSize( Size::ZERO ), _delegate( nullptr ), _runningSequence( nullptr ) {
        init();
    }
...
// refer to CCBReader::readNodeGraph() for data structure of pSeq
    void CCBAnimationManager::addNode( Node *pNode,
                                       const std::unordered_map<int, Map<std::string, CCBSequenceProperty *>> &seq ) {
        // pNode->retain();
...
// Refer to CCBReader::readKeyframe() for the real type of value
    ActionInterval *
    CCBAnimationManager::getAction( CCBKeyframe *pKeyframe0, CCBKeyframe *pKeyframe1, const std::string &propName,
                                    Node *pNode ) {
        ...
        if ( propName == "rotationX" ) {
            ...
            return Sequence::createWithTwoActions( DelayTime::create( duration ),
                                                   CCBSetSpriteFrame::create(
                                                           static_cast<SpriteFrame *>(pKeyframe1->getObject()) ) );
    ...
    void
    CCBAnimationManager::setAnimatedProperty( const std::string &propName, Node *pNode, const Value &value, Ref *obj,
        ...
        if ( fTweenDuration > 0 ) {
            ...
            if ( propName == "position" ) {
                ...
                pNode->setPosition(
                        getAbsolutePosition( Vec2( x, y ), type, getContainerSize( pNode->getParent() ), propName ) );
            } else if ( propName == "scale" ) {
                // Get scale type
                auto &array = getBaseValue( pNode, propName ).asValueVector();
                CCBReader::ScaleType type = ( CCBReader::ScaleType ) array[2].asInt();
    ...
    void CCBAnimationManager::setFirstFrame( Node *pNode, CCBSequenceProperty *pSeqProp, float fTweenDuration ) {
        ...
        if ( keyframes.empty() ) {
            ...
            setAnimatedProperty( pSeqProp->getName(), pNode, keyframe->getValue(), keyframe->getObject(),
                                 fTweenDuration );
        }
    }

    ActionInterval *CCBAnimationManager::getEaseAction( ActionInterval *pAction, CCBKeyframe::EasingType easingType,
                                                        float fEasingOpt ) {
    ...
    void CCBAnimationManager::runAnimationsForSequenceIdTweenDuration( int nSeqId, float fTweenDuration ) {
        ...
        Action *completeAction = Sequence::createWithTwoActions(
                DelayTime::create( seq->getDuration() + fTweenDuration ),
 ...
 ************************************************************/
    ...
    CCBSoundEffect *
    CCBSoundEffect::actionWithSoundFile( const std::string &filename, float pitch, float pan, float gain ) {
