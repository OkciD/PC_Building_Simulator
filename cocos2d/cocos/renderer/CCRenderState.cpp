...
NS_CC_BEGIN
    ...
    RenderState::RenderState()
            : _hash( 0 ), _hashDirty( true ), _parent( nullptr ), _texture( nullptr ) {
...
// _blendEnabled is TRUE
    RenderState::StateBlock::StateBlock()
            : _cullFaceEnabled( false ), _depthTestEnabled( true ), _depthWriteEnabled( false ),
              _depthFunction( RenderState::DEPTH_LESS ), _blendEnabled( true ), _blendSrc( RenderState::BLEND_ONE ),
              _blendDst( RenderState::BLEND_ZERO ), _cullFaceSide( CULL_FACE_SIDE_BACK ), _frontFace( FRONT_FACE_CCW ),
              _stencilTestEnabled( false ), _stencilWrite( RS_ALL_ONES ),
              _stencilFunction( RenderState::STENCIL_ALWAYS ), _stencilFunctionRef( 0 ),
              _stencilFunctionMask( RS_ALL_ONES ), _stencilOpSfail( RenderState::STENCIL_OP_KEEP ),
              _stencilOpDpfail( RenderState::STENCIL_OP_KEEP ), _stencilOpDppass( RenderState::STENCIL_OP_KEEP ),
              _bits( 0L ) {
    }
...
//    }
    ...
    static RenderState::StencilFunction parseStencilFunc( const std::string &value ) {
        ...
        if ( upper == "NEVER" )
            ...
            CCLOG( "Unsupported stencil function value (%s). Will default to STENCIL_ALWAYS if errors are treated as warnings)",
                   value.c_str() );
            return RenderState::STENCIL_ALWAYS;
        }
    }
    ...
    static RenderState::StencilOperation parseStencilOp( const std::string &value ) {
        ...
        if ( upper == "KEEP" )
            ...
            CCLOG( "Unsupported stencil operation value (%s). Will default to STENCIL_OP_KEEP if errors are treated as warnings)",
                   value.c_str() );
            return RenderState::STENCIL_OP_KEEP;
        }
    }
