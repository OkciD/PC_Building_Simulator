...
/** @script{ignore} */
void calculateNamespacePath( const std::string &urlString, std::string &fileString,
...
Properties::Properties( Data *data, ssize_t *dataIdx, const std::string &name, const char *id, const char *parentID,
                        Properties *parent )
        : _dataIdx( dataIdx ), _data( data ), _namespace( name ), _variables( NULL ), _dirPath( NULL ),
          _parent( parent ) {
    if ( id ) {
        _id = id;
    }
    ...
    while ( true ) {
        ...
        if ( comment ) {
            ...
            if ( rc != NULL ) {
                ...
                name = trimWhiteSpace( name );
                if ( name == NULL ) {
                    CCLOGERROR( "Error parsing properties file: failed to determine a valid token for line '%s'.",
                                line );
                    return;
                } else if ( name[0] == '}' ) {
                    // End of namespace.
                ...
                if ( value != NULL && value[0] == '{' ) {
                    ...
                    if ( rc != NULL ) {
                        ...
                        Properties *space = new( std::nothrow ) Properties( _data, _dataIdx, name, value, parentID,
                                                                            this );
                        _namespaces.push_back( space );
                        ...
                        if ( c == '{' ) {
                            ...
                            if ( seekFromCurrent( -1 ) == false )
                                CCLOGERROR(
                                        "Failed to seek backwards a single character after testing if the next line starts with '{'." );
...
bool Properties::parseColor( const char *str, Vec3 *out ) {
    if ( str ) {
        if ( strlen( str ) == 7 && str[0] == '#' ) {
            ...
            // Not a color string.
            CCLOGWARN( "Error attempting to parse property as an RGB color (not specified as a color string): %s",
                       str );
        }
    }
...
bool Properties::parseColor( const char *str, Vec4 *out ) {
    if ( str ) {
        if ( strlen( str ) == 9 && str[0] == '#' ) {
            ...
            // Not a color string.
            CCLOGWARN( "Error attempting to parse property as an RGBA color (not specified as a color string): %s",
                       str );
        }
    }
